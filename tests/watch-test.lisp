...
(def-test-method test-watch ((test watch-test))
  (let* ((watch-1 (make-instance 'watch))
    ...
    (sleep 0.01)
    (stop watch-1)
    (assert-true (< (- (lap watch-1) 10000000) 500000) "A")
    ...
    (sleep 0.01)
    (stop watch-1)
    (assert-true (< (- (lap watch-1) 10000000) 500000) "B")
    (reset watch-1)
    (reset watch-2 t)
    (assert-equal 0 (lap watch-1))
    (start watch-1)
    ...
    (sleep 0.01)
    (stop watch-1)
    (assert-true (< (- (lap watch-1) 10000000) 500000) "C")
    (assert-true (< (- (lap watch-2) 10000000) 500000) "D")))